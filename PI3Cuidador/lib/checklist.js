...
export default function Checklist({idosoId, idosoNome = 'Idoso'}) {
  ...
  const carregarChecklist = async (db) => {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível carregar a checklist: ' + error.message);
      // Usar dados padrão em caso de erro
      setSections(defaultData.map((section, index) => ({
  ...
  const addSection = async () => {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível adicionar a seção: ' + error.message);
    } finally {
      if (db) {
        try {
          await db.close();
  ...
  const addItemToSection = async (sectionId, sectionIndex) => {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível adicionar o item: ' + error.message);
    } finally {
      if (db) {
        try {
          await db.close();
  ...
  const deleteSection = async (index) => {
    ...
    try {
      ...
      await db.executeSql(
        'DELETE FROM checklist_items WHERE section_id = ?',
        [sectionId]
      );
      
      // Excluir a seção
      await db.executeSql(
        'DELETE FROM checklist_sections WHERE id = ?',
        [sectionId]
      );
  ...
  try {
    ...
    Alert.alert('Erro', 'Não foi possível resetar o checklist: ' + error.message);
  } finally {
    if (db) {
      try {
        await db.close();
  ...
  const scheduleAlarm = async (title, message) => {
    try {
      ...
      Alert.alert('Sucesso', `Alarme agendado para: ${date.toLocaleTimeString()}`);
    } catch (error) {
      console.error('Erro ao agendar notificação:', error);
      Alert.alert('Erro', 'Não foi possível agendar a notificação: ' + error.message);
    }
  };
      ...
      />
        ...
        <ScrollView>
          {sections.map((section, sectionIndex) => (
            <View key={section.id || section.title + sectionIndex} style={styles.section}>
                ...
                return (
                  <View key={item.id || key} style={styles.item}>
                    <TouchableOpacity
                      ...
                      onPress={() => toggleItem(section.id, item.id, section.title, item.text)}>
                      <Text
                        style={[
                          styles.checkbox,
